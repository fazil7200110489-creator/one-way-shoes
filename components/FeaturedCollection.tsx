...
export default function FeaturedCollection() {
  ...
  useEffect(() => {
    fetch("/api/products")
      .then(res => res.json())
      .then(data => {
        setFeatured(data.filter((s: any) => s.featured));
        setLoading(false);
      })
      .catch(console.error);
  }, []);

  if (loading) return <div className="py-20 flex justify-center"><div className="w-8 h-8 border-2 border-white/20 border-t-accent rounded-full animate-spin" /></div>;
